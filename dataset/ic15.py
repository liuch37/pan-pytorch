'''
This code is to build data loader for ICDAR-2015 dataset.
'''
...
def get_ann(img, gt_path):
    h, w = img.shape[0:2]
    #lines = mmcv.list_from_file(gt_path) # replaced
...
class PAN_IC15(data.Dataset):
    def __init__(self,
        ...
        for data_dir, gt_dir in zip(data_dirs, gt_dirs):
            ...
            img_names = [img_name for img_name in os.listdir(data_dir) if img_name.endswith('.jpg') or img_name.endswith('.JPG')]
...
if __name__ == '__main__':
    ...
    train_dataset = PAN_IC15(split='train',
                             is_transform=False,
                             img_size=None,
                             short_size=736,
                             kernel_scale=0.5,
    ...
    for i, data in enumerate(train_dataset):
        ...
        gt_words = gt_words.detach().cpu().numpy()
        word_masks = word_masks.detach().cpu().numpy()
        print(gt_words)
        print(word_masks)
        '''
        plt.figure(1)
        ...
        pdb.set_trace()
        '''
    test_dataset = PAN_IC15(split='test',
                            is_transform=False,
                            img_size=None,
                            short_size=736,
                            kernel_scale=0.5,
